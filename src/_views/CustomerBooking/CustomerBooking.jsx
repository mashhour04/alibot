...
class CustomerBookging extends Component {
  constructor() {
    super();
    const client = window.client;
    const session = window.client.session || {};
    this.state = {
      selectedTime: '',
  ...
  handleGettingAvailable = ({ dateOfBooking, numberOfPeople }) => {
    ...
    const vendorId = session.vendor._id;
    ...
    if (capacityValue && timestamp) {
      this.props.dispatch(
        vendorActions.getAvailableTables({
          timestamp,
  ...
  handleDateChange = ({ hours, minutes, date, target }) => {
    ...
    const selectedTime = target ? target.value : undefined;
    this.setState({
  ...
  handleSubmit = () => {
    let { availableTables } = this.props;
    const { name, capacity, selectedDate, email, client,session } = this.state;
    const vendorId = session.vendor._id;

    const table = this.getChoosenTable(availableTables, capacity);
    const tableId = (table && table !== '') ? table._id : null;
    if (!selectedDate) {
      return Toast.fire({
        type: "error",
  ...
  getChoosenTable(tables, capacity) {
    ...
    if (tables && tables.length) {
      let closest = tables.reduce(function(prev, curr) {
        const currValue = parseInt(curr.capacity, 10),
          prevValue = parseInt(prev.capacity, 10);
        console.log("prev", prevValue, prev);
        console.log(
          "current",
          currValue,
          "abs",
          Math.abs(currValue - capacity) < Math.abs(prevValue - capacity),
          curr,
          prev
        );
        return Math.abs(currValue - capacity) < Math.abs(prevValue - capacity)
          ? curr
          : prev;
      });
      return closest;
  ...
  render() {
    ...
    const { selectedDate, capacity, selectedTime, client, session } = this.state;
    const availableOptions = availableTables.length
      ? availableTables.map(table => {
          table.label =
            table.name && table.name !== "" ? table.name : table.altId;
        ...
        <Card>
          ...
          <CardBody>
              ...
              <GridItem xs={6} sm={6} md={6}>
                <CustomInput
                  ...
                  inputProps={{
                    name: "name",
                    type: "text",
                    value: window.client ? window.client.firstName : undefined,
            ...
            <GridContainer>
              ...
              ) : availableTables && availableTables.loading ? (
...
export default connect(mapStateToProps)(withStyles(styles)(CustomerBookging));
